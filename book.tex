...
\begin{proof} 
	...
	Множество $H_{\varepsilon}$ компактно в $C[a, b]$. Действительно, каждая ломаная определяется $n$ числами $(\eta_1, \eta_2,...,\eta_n )$, где все числа $\eta_k$ ограничены: $|\eta_k|\leqslant const$. Из любой последовательности ломаных из $H_{\varepsilon}$ можно образовать фундаментальную последовательность.
...
\begin{enumerate}
	...
	\item $\lVert x\rVert \geqslant 0$; если $\lVert x\rVert = 0$, то $x=\circleddash$.
...
\begin{enumerate}
	...
	\item Пространство Банаха $\mathcal{L}_p(a,b)$ измеримых и суммируемых со степенью $p,\;p\geq1,$ функций. Норма:
...
\parallel x \parallel \cdot \parallel A(\frac{x}{\parallel x \parallel}) \parallel \leqslant C_0$
\end{center} 
при $\parallel x \parallel \leqslant 1$; 
$\underset{\parallel x\parallel \leqslant 1}{\sup}\parallel Ax \parallel \leqslant C_0$.

Тогда 
\begin{center}
$ C_0 \geqslant\underset{\parallel x\parallel \leqslant 1}{\sup}\parallel Ax \parallel $, и мы получаем 
$\underset{\parallel x\parallel \leqslant 1}{\sup}\parallel Ax \parallel =\parallel A \parallel$.
\end{center}

Для линейных операторов верна оценка: $\parallel Ax \parallel _{Y} =\parallel A \parallel \cdot \parallel x \parallel_{X} $.

\textbf{Замечание.} Если получена оценка  $\parallel Ax \parallel \leqslant {C}  \parallel x \parallel$, то $\parallel A \parallel \leqslant {C} $.
...
$= \parallel x \parallel _{\mathcal{L}_p(a,b)}\cdot (\displaystyle\int\limits_a^b {\vert K(t,\tau)\vert} ^q d\tau)^\frac{1}{q}$.

${\parallel y \parallel}^q _{\mathcal{L}_q(a,b)}=\displaystyle\int\limits_a^b {\vert \displaystyle\int\limits_a^b K(t,\tau)\cdot x(\tau)d\tau \vert^{q}} dt\leq {\parallel x \parallel}^q _{\mathcal{L}_p(a,b)} \cdot \displaystyle\int\limits_a^b dt \displaystyle\int\limits_a^b {\vert K(t,\tau)\vert} ^q d\tau$.

${\parallel y \parallel}_{\mathcal{L}_q(a,b)}=
{\parallel Kx\parallel}_{\mathcal{L}_q(a,b)} \leqslant {(\displaystyle\int\limits_a^b \displaystyle\int\limits_a^b {\vert K(t,\tau)\vert} ^q d\tau dt)^\frac{1}{q}}\cdot \parallel x \parallel _{\mathcal{L}_p(a,b)} $ 
\end{center}
и $\parallel K \parallel \leqslant {(\displaystyle\int\limits_a^b \displaystyle\int\limits_a^b {\vert K(t,\tau)\vert} ^q d\tau dt)^\frac{1}{q}}$.
...
Ясно, что выполнены все аксиомы линейного множества, множество $\mathcal{L}(X,Y)$ - линейное множество. Введем метрику в этом множестве $\rho(A, B) = \parallel A-B\parallel$ и получим нормированное пространство $\mathcal{L}(X,Y)$. Проверим, например, выполнение неравенства треугольника. Согласно определению $(A+B)x = Ax+Bx$ и $\parallel(A+B)x\parallel\le(\parallel A\parallel+\parallel B\parallel)\parallel x\parallel$. Следовательно $\parallel A+B\parallel\le\parallel A\parallel+\parallel B\parallel$.
...
$\parallel A_n\parallel=\sup \limits_{S_1} \parallel A_nx\parallel=\displaystyle\sum_{k=1}^\infty \lvert x_k\rvert =1$, $\parallel A_n\parallel=1$, $A_n\in \mathcal{L}(l_1,l_1)$.

Рассмотрим последовательность линейных операторов $E-A_n\in \mathcal{L}(l_1,l_1)$. Ясно, что $\parallel(E-A_n)x\parallel=\parallel(0,0,...,0,x_{n+1},x_{n+2},...)\parallel=\displaystyle\sum_{k=n+1}^\infty \lvert x_k\rvert  \to 0$ при $n\to \infty$, так как числовой ряд $\displaystyle\sum_{k=1}^\infty \lvert x_k\rvert $ сходится.
...
$2c \geqslant \frac{r}{\lVert x\rVert}\lVert A_n x\rVert$, $\frac{2c}{r}\lVert x\rVert \geqslant \lVert A_n x\rVert$, $\lVert A_n x\rVert \le \frac{2c}{r}\lVert x\rVert$
\end{center}
...
\begin{center}
	$\parallel By \parallel \leqslant C_0 \parallel y \parallel $
\end{center}
\begin{enumerate}
	...
	$y_1 \in M$:
	\begin{center}
	$\parallel y_1 - \frac{3}{4}y_0\parallel \leqslant \frac{1}{4}    		\parallel y_0 \parallel $,
	\end{center}
	\begin{center}
	$\parallel y_1 \parallel =
	\parallel y_1 - \frac{3}{4}y_0 +
	\frac{3}{4}y_0 \parallel \leqslant \frac{1}{4} 			\parallel y_0 \parallel +
	...
	\frac{1}{4}y_0 \parallel \leqslant
	...
	\frac{1}{4}(y_1 - y_0) \parallel \leqslant
	\frac{1}{4} \parallel y_1 - y_0 \parallel +
	\frac{1}{4} \parallel y_1 - y_0 \parallel = $
	\end{center}
	\begin{center}
	$ = \frac{1}{2} \parallel y_1 - y_0 \parallel 			\leqslant
	\frac{1}{2^2} \parallel y_0 \parallel$ ,
	\end{center}
	\begin{center}
	$\parallel y_2 \parallel =
	\parallel y_2 - \frac{3}{4}(y_1 - y_0) +
	\frac{3}{4}(y_1 - y_0) \parallel \leqslant
	\frac{1}{4} \parallel y_1 - y_0 \parallel +
	\frac{3}{4} \parallel y_1 - y_0 \parallel =
	 \parallel y_1 - y_0 \parallel \leqslant$
	 \end{center}
	\begin{center}
	 $ \leqslant \frac{1}{2} \parallel y_0 \parallel$
	\end{center}
	
	\item Рассмотрим элемент $y_2+y_1- y_0$ . Для него 	\begin{center}	
	$\parallel y_2+y_1- y_0 \parallel \leqslant
	\frac{1}{2^2} \parallel y_0 \parallel$ .
	\end{center}
	...
	\frac{1}{4}(y_0 - y_1 - y_2) \parallel \leqslant $
	\end{center}
	\begin{center}
	$\leqslant \frac{1}{4} \parallel y_0 - y_1 - y_2 \parallel +
	\frac{1}{4} \parallel y_0 - y_1 - y_2 \parallel 			\leqslant $
	\end{center}
	\begin{center}
	...
	\parallel y_3 - \frac{3}{4}(y_0 - y_1 - y_2) +
	\frac{3}{4}(y_0 - y_1 - y_2) \parallel \leqslant $
	\end{center}
	\begin{center}
	$\leqslant \frac{1}{4} \parallel y_0 - y_1 - y_2 				\parallel +
	\frac{3}{4} \parallel y_0 - y_1 - y_2 \parallel =
	 \parallel y_0 - y_1 - y_2 \parallel \leqslant
	 \frac{1}{2^2} \parallel y_0 \parallel$
	\end{center}
...
$\lVert y_n + y_{n-1}+y_{n-2}+...+y_2+y_1+y_0 \rVert \leqslant \frac{1}{2^n} \lVert y_0 \rVert.$
\end{center}
...
\begin{center}
$ \lVert B s_{n+m}-B s_n \rVert _X = \lVert B(s_{n+m}-s_n) \rVert= \lVert B(y_{n+1}+y_{n+2}+...+y_{n+m}) \rVert \leqslant $
\end{center}

\begin{center}
$\leqslant C_0 \lVert y_0 \rVert (\frac{1}{2^n} + \frac{1}{2^{n+1}}+...+\frac{1}{2^{n+m-1}})=C_0 \frac{1}{2^n} (1 + \frac{1}{2}+...+\frac{1}{2^{m-1}}) \to 0$ при $n \to \infty$.
\end{center}
...
\begin{center}
$ \lVert x^*-B s_n \rVert \leqslant C_0 \lVert y_0\rVert (\frac{1}{2^n} + \frac{1}{2^{n+1}}+...)$.
\end{center}
...
\begin{center}
$\leqslant C_0 \lVert y_0\rVert [\frac{1}{2^n} + \frac{1}{2^{n+1}}+...] + C_0 \lVert y_0\rVert [1+\frac{1}{2} +\frac{1}{2^2}+ ...+\frac{1}{2^{n-1}}]= 2 C_0 \lVert y_0\rVert$
\end{center}
...
Для нормы функционала $f$: $\lvert  \langle x, f \rangle \rvert \le \lVert y\rVert\lVert x\rVert$. Тогда $\lVert f\rVert \leqslant \lVert y\rVert$. С другой стороны $ \langle y, f \rangle  = (y, y) \leqslant \lVert f\rVert\lVert y\rVert$, $\lVert y\rVert\le\lVert f\rVert$
...
Обозначим множество элементов пространства $H$, таких что ${\parallel x \parallel} _H \leqslant C$ через $K$. Пусть $\lbrace x_n \rbrace$ - произвольная последовательность элементов множества $K$.
...
\begin{center}
$\vert f_1(x_n)\vert = \vert \langle x,\Psi_1 \rangle \vert \leqslant \parallel x_n\parallel \leqslant C$.
\end{center}
...
$$\parallel K \parallel = \Big( \displaystyle\int\limits_a^b \lvert K(t,\tau) \rvert^q d\tau dt  \Big)^{\frac{1}{q}}$$

$$\parallel K^{*} \parallel =  \Big( \displaystyle\int\limits_a^b \lvert K(\tau, t) \rvert^q dt d\tau  \Big)^{\frac{1}{q}} =  \Big( \displaystyle\int\limits_a^b \lvert K(t,\tau) \rvert^q d\tau dt  \Big)^{\frac{1}{q}}$$
