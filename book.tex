...
\begin{enumerate}
	...
	$$ \parallel x \parallel^2 = \displaystyle\int\limits_a^b \varphi(t) \lvert x(t) \rvert^2 dt$$
...
\begin{theorem}
	...
	где $c_k = (x, \varphi_k)$ и $\displaystyle\sum_{k} \lvert c_k \rvert^2 = \parallel x \parallel^2$ ($c_k$ - коэффициенты Фурье элемента $x$ в базисе $\varphi_k$).
\end{theorem}
...
Пусть оператор $A \subset L(X,Y)$, функционал $f \in Y^*$. Вычислим значения $ \langle Ax,f \rangle $. Эти значения можно трактовать как значения функционала $\varphi$, определённого на элементах пространства $X$:
\begin{equation}
...
\begin{center}
$\lVert \varphi \lVert \leq \underset{\lVert x\lVert =1}{\sup} 
\vert \varphi(x) \vert \leq  \underset{\lVert x\lVert =1}{\sup} 
...
\begin{center}
$\lVert \varphi \lVert = \lVert A^* \cdot f \lVert \leq \lVert A \lVert \cdot \lVert f \lVert$
\end{center}
\begin{center}
