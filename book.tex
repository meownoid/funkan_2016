...
% You need cm-super package to get beautifull vector fonts
...
\begin{equation*}
x_{n_2} \in V_{\varepsilon _1} (z_1) \cap V_{\varepsilon _2} (z_2),
\end{equation*}
...
Для линейных операторов верна оценка: $\Vert Ax \Vert _{Y} \leqslant \Vert A \Vert \cdot \Vert x \Vert_{X} $.
...
\begin{proof}
\underbar{Необходимость}. Пусть $A_n x \to A x$. Тогда $\lVert A_n x\rVert \to \lVert A x\rVert \leqslant \lVert A\rVert \lVert x\rVert$, и последовательность норм элементов $\lVert A_n x\rVert$ ограничена при каждом $x \in X$. По лемме (\rom{2}) нормы $\lVert A_n\rVert$ ограничены в совокупности --- условие 1 выполнено.
...
Легко получить необходимое условие этого <<элементарного>> расширения. Для построения элементарного продолжения необходимо априори задать элемент $y_1 = A_1 x_1^0$:
...
Пусть $X_0$ --- подпространство пространства $X$, и на $X_0$ определён функционал $f_0$ с нормой $\lVert f_0 \rVert = \sup \limits_{\lVert x \rVert=1, x \in X_0} \lvert f_0(x) \rvert$. Пусть элемент $x_1^0 \in X$, но $x_1^0 \notin X_0$.
...
\begin{definition}
Если для всех элементов множества $E$ верно неравенство $f(x) \leqslant C$ (или $f(x) \geqslant C$), то говорят, что $E$ \textit{лежит по одну сторону от плоскости} $L$. Плоскость $L$ называется \textit{опорной} для множества $E$ в точке $x_0 \in E$, если $E$ лежит по одну сторону от $L$ и $x_0 \in L$.
\end{definition}
...
\begin{align*}
\langle Kx,f \rangle &\in L_q(a,b)=\int\limits_a^b (Kx)(t)f(t)dt = \mbox{(где $f \in L_p(a,b)$)} = \\
...
\begin{proof}
Так как $\{x_n\}\to x_0$, то $\{\lVert x_n\rVert\}$ ограничена (\S 4.1). Из полной непрерывности оператора $A$ следует, что последовательность элементов $y_n = Ax_n$ \underbar{компактна}.
