...
\end{corollary}
\begin{proof}
Обозначим линейную оболочку системы элементов $\{ x_2, x_3, ... x_n\}$ через $\Omega_1$, $\Omega_1 \subset X$. Элемент $x_1 \overline{\in} \Omega_1$: $\rho(x_1, \Omega)=d_1>0$. Согласно свойству (2) существует функционал $f_1 \in X^*$, такой, что 
\begin{equation*}
...
\begin{theorem}
Для любого замкнутого шара $\overline{V_r}(\circleddash)$, ($\lVert x \rVert \leq r$) и для любой точки $x_0 \in S_r(\circleddash)$ существует опорная плоскость $L$ к шару $\overline{V_r}(\circleddash)$ в точке $x_0$.
\end{theorem}
...
\end{equation*}
Продолжим функционал $f_0$ на все пространство $X$ с сохранением нормы. Опорная плоскость к шару $\overline{V_r}(\circleddash)$ в точке $x_0$ определяется уравнением $f(x)=r$. Действительно, для $x \in \overline{V_r}(\circleddash)$:
\begin{equation*}
...
\begin{proof}
Проведем доказательство для сепарабельных гильбертовых пространств. В этих пространства существует не более чем счетный базис элементов $\psi_1, \psi_2, ..., \psi_n, ...$, $(\psi_i, \psi_j)=0, i\neq j$; ${\lVert \psi_i \rVert}_H=1$.
\end{proof}
\end{theorem}
...
\begin{equation*}
\sum\limits_{k=1}^n{[ \langle x_{N(m,m)},\Psi_k \rangle  - \alpha_k]}^2 < \epsilon^2.
\end{equation*}
Таким образом
\begin{equation*}
...
$$ f(x) = \langle x, f \rangle = (x,y) = \displaystyle\int\limits_a^b x(t)\overline{y(t)}dt $$
$$ (\frac{1}{p} + \frac{1}{q} = 1) $$
$$ \langle x, f \rangle  = \bar{(y,x)}  = \displaystyle\int\limits_a^b \overline{(\bar{x}(t)y(t))} dt = \overline{\displaystyle\int\limits_a^{-b} (y(t)\bar{x}(t))dt} $$
